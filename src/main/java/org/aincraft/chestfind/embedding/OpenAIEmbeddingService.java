...
public class OpenAIEmbeddingService implements EmbeddingService {
    ...
    public OpenAIEmbeddingService(JavaPlugin plugin, ChestFindConfig config) {
        this.plugin = plugin;
        this.config = config;
    ...
    public CompletableFuture<Void> initialize() {
        return CompletableFuture.runAsync(() -> {
            ...
            plugin.getLogger().info("OpenAI embedding service initialized with model: " + config.getOpenAIModel());
        }, executor);
    }
    ...
    public CompletableFuture<float[]> embed(String text, String taskType) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                plugin.getLogger().log(Level.WARNING, "Failed to embed text with OpenAI", e);
