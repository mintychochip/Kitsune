...
public class ItemSerializer {
    ...
    private static void addSemanticTags(StringBuilder sb, String materialName, JsonObject itemJson) {
        ...
        // Category-based tags (already in JSON from createItemJson)
        // These include: weapon, tool, armor, food, block, raw_material, magical, decorative, etc.
    ...
    /**
     * Legacy method - serialize all items into single JSON (deprecated)
     * @deprecated Use serializeItemsToChunks for chunked output
     */
    @Deprecated
    public static String serializeItems(ItemStack[] items) {
        ...
        for (ItemStack item : items) {
            ...
            itemsArray.add(createItemJson(item));
        }

        JsonObject container = new JsonObject();
        container.add("items", itemsArray);
    ...
    /**
     * Generate a natural language summary of container contents.
     * Highlights enchanted and valuable items first for better embedding quality.
     */
    private static String generateNaturalLanguageSummary(JsonArray itemsArray) {
        int totalItems = 0;
        ...
        for (int i = 0; i < itemsArray.size(); i++) {
            ...
            // Track item counts
            itemCounts.merge(name, amount, Integer::sum);

            // Highlight enchanted items first, then valuable items, then ALL items
            if (item.has("enchantments")) {
                String desc = amount + " " + name + " (" + item.get("enchantments").getAsString() + ")";
                highlights.add(desc);
            } else {
                // Add ALL items to highlights (not just valuable ones)
                highlights.add(amount + " " + name);
            }
        }

        // Sort highlights by descending amount (most valuable first)
        highlights.sort((a, b) -> {
        ...
        // Build simple plain text list - no JSON, no complex formatting
        // Format: "Item1, Item2, Item3" for optimal embedding
        StringBuilder summary = new StringBuilder();

        // Include ALL items for better keyword matching
        int highlightCount = Math.min(15, highlights.size()); // Show more items
        for (int i = 0; i < highlightCount; i++) {
            if (i > 0) summary.append(", ");
            summary.append(highlights.get(i));
        }

        if (highlights.size() > 15) {
            summary.append(", and ").append(highlights.size() - 15).append(" more items");
        }
    ...
    /**
     * Check if an item is considered valuable for highlighting.
     */
    private static boolean isValuableItem(String itemName) {
        String lowerName = itemName.toLowerCase();
        return lowerName.contains("diamond") ||
               lowerName.contains("netherite") ||
               lowerName.contains("emerald") ||
               lowerName.contains("gold") ||
               lowerName.contains("ancient debris");
    }

    /**
     * Extract the leading amount from a highlight string (e.g., "64 diamond" -> 64).
     */
    private static int extractAmount(String highlight) {
     ...
     * Creates distinct embeddings for "Diamond Pickaxe" vs "Diamond Sword"
     * while keeping "Diamond Pickaxe" and "Diamond Pickaxe" very similar.
     */
    ...
    /**
     * Add material properties that help distinguish different item types.
     */
    ...
    /**
     * Add specific tool type metadata to distinguish between different tools.
     */
    private static void addToolTypeMetadata(JsonObject itemObj, String materialName) {
        ...
        // Tool material (diamond, iron, stone, wooden, golden)
    ...
    /**
     * Add functional properties to better categorize items.
     */
    ...
    /**
     * Extract ALL data components from an ItemStack.
     */
    ...
    /**
     * Extract enchantments (ENCHANTMENTS and STORED_ENCHANTMENTS).
     */
    ...
    /**
     * Extract custom names (CUSTOM_NAME and ITEM_NAME).
     */
    ...
    /**
     * Extract durability info (DAMAGE and MAX_DAMAGE).
     */
    ...
    /**
     * Extract modifiers (UNBREAKABLE, ATTRIBUTE_MODIFIERS, etc.).
     */
    ...
    /**
     * Extract appearance components (DYED_COLOR, TRIM).
     */
    ...
    /**
     * Extract specialized components (POTION_CONTENTS, BUNDLE_CONTENTS, etc.).
     */
    ...
    /**
     * Categorize nutrition value into semantic categories.
     */
    ...
    /**
     * Categorize durability into condition categories.
     */
    ...
    /**
     * Categorize mining speed into performance categories.
     */
