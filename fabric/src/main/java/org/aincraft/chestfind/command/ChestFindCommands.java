...
/**
 * Brigadier commands for ChestFind.
 */
public class ChestFindCommands {
    private ChestFindCommands() {
    }

    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, ChestFindMod mod) {
        // /find <query>
        dispatcher.register(
                CommandManager.literal("find")
                        .then(CommandManager.argument("query", StringArgumentType.greedyString())
                                .executes(ctx -> executeSearch(ctx, mod,
                                        ...
                                        mod.getChestFindConfig().getDefaultSearchLimit())))
                        .executes(ctx -> {
        ...
        dispatcher.register(
                CommandManager.literal("chestfind")
                        .then(CommandManager.literal("reload")
                                .requires(source -> source.hasPermissionLevel(2))
                                .executes(ctx -> {
                                    mod.reloadConfig();
                                    ctx.getSource().sendFeedback(() ->
                                            Text.literal("ChestFind config reloaded.").formatted(Formatting.GREEN), true);
                                    return 1;
                                }))
                        ...
                        .executes(ctx -> {
                            ctx.getSource().sendFeedback(() -> Text.literal("ChestFind Commands:")
                            ...
                            ctx.getSource().sendFeedback(() -> Text.literal("/chestfind reload")
                                    .formatted(Formatting.GRAY)
                                    .append(Text.literal(" - Reload config").formatted(Formatting.WHITE)), false);
                            ctx.getSource().sendFeedback(() -> Text.literal("/chestfind stats")
                                    .formatted(Formatting.GRAY)
                                    .append(Text.literal(" - Show statistics").formatted(Formatting.WHITE)), false);
                            ctx.getSource().sendFeedback(() -> Text.literal("/chestfind reindex <radius>")
                                    .formatted(Formatting.GRAY)
                                    .append(Text.literal(" - Reindex nearby").formatted(Formatting.WHITE)), false);
                            ctx.getSource().sendFeedback(() -> Text.literal("/chestfind purge")
                                    .formatted(Formatting.GRAY)
    ...
    private static int executeSearch(CommandContext<ServerCommandSource> ctx, ChestFindMod mod, String query, int limit) {
        ServerCommandSource source = ctx.getSource();

        if (!mod.isInitialized()) {
            source.sendFeedback(() ->
                    Text.literal("ChestFind is still initializing. Please wait...").formatted(Formatting.RED), false);
            return 0;
        }
        ...
        mod.getEmbeddingService().embed(query, "RETRIEVAL_QUERY")
                ...
                .exceptionally(ex -> {
                    mod.getElfPlugin().getLogger().log(Level.WARNING, "Search failed", ex);
                    source.sendFeedback(() ->
    ...
    private static int executeStats(CommandContext<ServerCommandSource> ctx, ChestFindMod mod) {
        ServerCommandSource source = ctx.getSource();

        if (!mod.isInitialized()) {
            source.sendFeedback(() ->
                    Text.literal("ChestFind is still initializing...").formatted(Formatting.RED), false);
            return 0;
        }

        mod.getVectorStorage().getStats().thenAccept(stats -> {
            source.sendFeedback(() -> Text.literal("ChestFind Stats:").formatted(Formatting.GOLD), false);
    ...
    private static int executeReindex(CommandContext<ServerCommandSource> ctx, ChestFindMod mod, int radius) {
        ServerCommandSource source = ctx.getSource();
        ServerPlayerEntity player = source.getPlayer();
    ...
    private static int executePurge(CommandContext<ServerCommandSource> ctx, ChestFindMod mod) {
        ServerCommandSource source = ctx.getSource();
